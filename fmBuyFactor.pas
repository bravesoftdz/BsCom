...
type
  TfmBuyFactor = class(TForm)
    shpBanner: TAddShape;
    AddImage1: TAddImage;
    lblTitle: TStdlabel;
    grdFactors: TDCDBGrid;
    ...
    cmbPayKind: TStdComboBox;
    lbledGoodsName: TAddLabeledEdit;
    lbledGFactorCode: TAddLabeledEdit;
    lbledGoodsPrice: TAddLabeledEdit;
    Stdlabel3: TStdlabel;
    chbFactorKind: TStdCheckBox;
    Stdlabel9: TStdlabel;
    btnCalc: TAddBitBtn;
    ...
    lbledUserName: TAddLabeledEdit;
    btnSelUsername: TAddBitBtn;
    btnSelGFactor: TAddBitBtn;
    spedCount: TSpinEdit;
    Stdlabel1: TStdlabel;
    lbledPFactoreCode: TAddLabeledEdit;
    btnSelPFactor: TAddBitBtn;
    lbledBankTell: TAddLabeledEdit;
    ...
    procedure cmbPayKindChange(Sender: TObject);
    ...
    procedure chbFactorKindClick(Sender: TObject);
  private
...
procedure TfmBuyFactor.FormShow(Sender: TObject);
begin
...
procedure TfmBuyFactor.lbledSelGoodsKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
     if Key= VK_DOWN Then
...
procedure TfmBuyFactor.LoadDevice(ForInsert: Boolean;deviceCode:Integer);
begin
...
procedure TfmBuyFactor.SelectedFieldExe(ForInsert: Boolean);
begin
...
procedure TfmBuyFactor.btnSelGoodsClick(Sender: TObject);
begin
     SelectedFieldExe(True);     
end;

procedure TfmBuyFactor.btnSelGoodsEditClick(Sender: TObject);
begin
     SelectedFieldExe(False);
end;

procedure TfmBuyFactor.lbledSelGoodsEditKeyUp(Sender: TObject;
...
procedure TfmBuyFactor.lbledSelGoodsChange(Sender: TObject);
begin
     if Trim(lbledSelGoods.Text) = '' Then Exit;
     LoadDevice(True,StrToint(lbledSelGoods.Text));
end;

procedure TfmBuyFactor.FormKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TfmBuyFactor.lbledSelGoodsEditChange(Sender: TObject);
begin
     if Trim(lbledSelGoodsEdit.Text) = '' Then Exit;
     LoadDevice(False,StrToint(lbledSelGoodsEdit.Text));
end;

procedure TfmBuyFactor.FormClose(Sender: TObject; var Action: TCloseAction);
begin
     with fmDMmain do
     begin
          tblPerson.Filtered := False;
...
procedure TfmBuyFactor.btnSelSellerEditClick(Sender: TObject);
begin
...
procedure TfmBuyFactor.lbledSellerEditKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
     if Key=VK_DOWN Then
     begin
...
procedure TfmBuyFactor.lbledSellerEditChange(Sender: TObject);
begin
     if Trim(lbledSellerEdit.Text) <> '' Then
        fmDMmain.tblPerson.Locate('xPersonCode_Pk_AN',StrToInt(lbledSellerEdit.Text),[]);
end;

procedure TfmBuyFactor.btnAddToListEditClick(Sender: TObject);
begin
...
function TfmBuyFactor.Calc(ForInsert: Boolean;ordercode:Integer): String;
Var
   thisQry : TMADOQuery ;
begin
...
function TfmBuyFactor.CalcMande(ForInsert: Boolean): String;
var
   thisResult : Integer;
begin
     thisResult := 0;
...
function TfmBuyFactor.CheckData(ForInsert: Boolean): Boolean;
begin
     Result := True;
     if ForInsert Then
     begin
...
procedure TfmBuyFactor.ClearAllTextBox;
Var
   i:Integer;
begin
...
procedure TfmBuyFactor.LoadDate(OrderCode: Integer);
Var
   thisGpCode:Integer;
begin
...
procedure TfmBuyFactor.lbledSellerChange(Sender: TObject);
begin
     if Trim(lbledSeller.Text) <> '' Then
        fmDMmain.tblPerson.Locate('xPersonCode_Pk_AN',StrToInt(lbledSeller.Text),[]);
end;

procedure TfmBuyFactor.lbledSellerKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
     if Key=VK_DOWN Then
     begin
...
procedure TfmBuyFactor.btnSelSellerClick(Sender: TObject);
begin
      SelectedField.FormCaption :='«‰ Œ«» ›—Ê‘‰œÂ';
      SelectedField.Query :='Select xPersonCode_Pk_AN,xIsSeller,(xName+'+QuotedStr(' ')+'+xFamily) As NameFamily From tblPerson Where xIsSeller=True Order By (xName+'+QuotedStr(' ')+'+xFamily)';
      SelectedField.DisplayFieldName := 'NameFamily';
...
procedure TfmBuyFactor.btnAddToListClick(Sender: TObject);
begin
     with TDefBuyGoodsOrderClass do
     begin
...
procedure TfmBuyFactor.lbledFactorNoChange(Sender: TObject);
begin
...
procedure TfmBuyFactor.btnDelFromListClick(Sender: TObject);
begin
     if fmDMmain.YesNoDialog.Show('¬Ì« «ÿ„Ì‰«‰ œ«—Ìœ ø',HbtInformation) Then
        fmDMmain.tblOrderGoodsList.Delete;
end;

procedure TfmBuyFactor.btnCalcClick(Sender: TObject);
Var
   TotalPrice:String;
begin
...
procedure TfmBuyFactor.BtnSaveClick(Sender: TObject);
begin
     case cmbpardakhtKind.ItemIndex of
...
procedure TfmBuyFactor.BtnSaveNewClick(Sender: TObject);
begin
     case cmbpardakhtKind.ItemIndex of
        0:NlbledMande.Text:= CalcMande(True);
        1:AlbledMande.Text:= CalcMande(True);
...
procedure TfmBuyFactor.btnDelClick(Sender: TObject);
begin
     if fmDMmain.YesNoDialog.Show('¬Ì« «ÿ„Ì‰«‰ œ«—Ìœ ø',HbtInformation) Then
        fmDMmain.tblOrder.Delete;
end;

procedure TfmBuyFactor.lbledSelorderKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
     if Key= Vk_down Then
     begin
...
procedure TfmBuyFactor.btnselOrderClick(Sender: TObject);
begin
      SelectedField.FormCaption      := '‘„«—Â ›«ò Ê— Œ—Ìœ';
      SelectedField.Query            := ' select xSellFactorNo,xOrderNo_Pk_AN,xIsSystemOrder,xBuyOrderFactorNo From tblOrder Where xBuyOrderFactorNo <> Null and xIsSystemOrder = False ';
      SelectedField.DisplayFieldName := 'xBuyOrderFactorNo';
...
procedure TfmBuyFactor.lbledSelorderChange(Sender: TObject);
begin
     if Trim(lbledSelorder.Text)='' Then Exit;
     LoadDate(StrToInt(lbledSelorder.Text));
end;

procedure TfmBuyFactor.ClearGoodsInfo(ForInsert:Boolean);
begin
     if ForInsert Then
     begin
          lbledSelGoods.Clear;
...
procedure TfmBuyFactor.btnDelFromListEditClick(Sender: TObject);
begin
     if fmDMmain.YesNoDialog.Show('¬Ì« «ÿ„Ì‰«‰ œ«—Ìœ ø',HbtInformation) Then
        fmDMmain.tblOrderGoodsList.Delete;
end;

procedure TfmBuyFactor.btnCalcEditClick(Sender: TObject);
Var
   TotalPrice:String;
begin
     if Trim(lbledSelorder.Text)='' Then Exit;
...
procedure TfmBuyFactor.NlbledTakhfifEditExit(Sender: TObject);
begin
     if NlbledTakhfifEdit.Text = '' Then
        NlbledTakhfifEdit.Text := '0';
end;

procedure TfmBuyFactor.AlbledTakhfifEditExit(Sender: TObject);
begin
     if AlbledTakhfifEdit.Text = '' Then
        AlbledTakhfifEdit.Text := '0';
end;

procedure TfmBuyFactor.ClbledTakhfifEditExit(Sender: TObject);
begin
     if ClbledTakhfifEdit.Text = '' Then
        ClbledTakhfifEdit.Text := '0';
end;

procedure TfmBuyFactor.cmbpardakhtKindEditChange(Sender: TObject);
Var
   TotalPrice:String;
begin
...
procedure TfmBuyFactor.cmbPayKindChange(Sender: TObject);
var
   totalPrice:String;
begin
     if thisOrderCode <> 0 then
...
procedure TfmBuyFactor.BtnCloseClick(Sender: TObject);
begin
     Close;
end;

procedure TfmBuyFactor.lbledGoodsCountKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TfmBuyFactor.tbsEditEnter(Sender: TObject);
begin
     lbledSelorder.SetFocus;
end;

procedure TfmBuyFactor.redOrderCommentsEditEnter(Sender: TObject);
begin
     redCommentsEdit.SelStart :=0;
end;

procedure TfmBuyFactor.redVarantyCommentEditEnter(Sender: TObject);
begin
     redVarantyCommentEdit.SelStart := 0;
end;

procedure TfmBuyFactor.btnSaveEditClick(Sender: TObject);
begin
     case cmbpardakhtKindEdit.ItemIndex of
...
function TfmBuyFactor.CheckGoodsData(ForInsert: Boolean): Boolean;
begin
     Result := True;
     if ForInsert Then
     begin
...
procedure TfmBuyFactor.ClbledTakhfifExit(Sender: TObject);
begin
     if ClbledTakhfif.Text = '' Then
        ClbledTakhfif.Text := '0';
end;

procedure TfmBuyFactor.AlbledTakhfifExit(Sender: TObject);
begin
     if AlbledTakhfif.Text = '' Then
        AlbledTakhfif.Text := '0';
end;

procedure TfmBuyFactor.NlbledTakhfifExit(Sender: TObject);
begin
     if NlbledTakhfif.Text = '' Then
        NlbledTakhfif.Text := '0';
end;

procedure TfmBuyFactor.clearAllDate;
Var
   i:Integer;
begin
     for i:=0 to componentcount-1 Do
...
procedure TfmBuyFactor.btnDefSellerEditClick(Sender: TObject);
begin
     lbledSellerEdit.Text := fmDefSeller.MyShowModal();
end;

procedure TfmBuyFactor.btnDefSellerClick(Sender: TObject);
begin
     ShowMessage(' ');
     lbledSeller.Text := fmDefSeller.MyShowModal();
end;

procedure TfmBuyFactor.AddBitBtn1Click(Sender: TObject);
begin
     ShowMessage(' ');
end;

procedure TfmBuyFactor.chbFactorKindClick(Sender: TObject);
begin
     case chbFactorKind.Checked of
          False: chbFactorKind.Caption := '›—Ê‘';
          True : chbFactorKind.Caption := 'Œ—Ìœ';
     end;
end;
