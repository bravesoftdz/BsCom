...
uses
  ...
  DCDBText, ImgList, Win32ImageList,HbtMessageDialog, AddImage, HbtXPEdit,
  HbtNumEdit;

type
  TfmDefCustomer = class(TForm)
    shpBaner: TAddShape;
    Stdlabel1: TStdlabel;
    dbgPerson: TDCDBGrid;
...
procedure TfmDefCustomer.FormShow(Sender: TObject);
begin
...
procedure TfmDefCustomer.BtnCloseClick(Sender: TObject);
begin
     Close;
end;

procedure TfmDefCustomer.FormClose(Sender: TObject; var Action: TCloseAction);
begin
     with fmDMmain do
     begin
...
function TfmDefCustomer.CheckData(ForInsert: Boolean): Boolean;
begin
     Result := True;
     if ForInsert Then
     begin
...
procedure TfmDefCustomer.ClearAllTextBox;
Var
   I:Integer;//For Index
begin
...
procedure TfmDefCustomer.LoadData(PersonCode: Integer);
begin
...
procedure TfmDefCustomer.chbIsMarridClick(Sender: TObject);
begin
...
procedure TfmDefCustomer.chbGendorClick(Sender: TObject);
begin
...
procedure TfmDefCustomer.FormKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TfmDefCustomer.BtnInsertClick(Sender: TObject);
begin
     lbledName.SetFocus;
...
procedure TfmDefCustomer.btnDelClick(Sender: TObject);
begin
     with fmDMmain.tblPerson Do
     begin
...
procedure TfmDefCustomer.lbledSelPersonKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
     if key = VK_DOWN Then
     begin
          SelectedField.Query :='Select xPersonCode_Pk_AN,(xName+'+QuotedStr(' ')+'+xFamily) As NameFamily,xIsSeller From tblPerson Where xIsSeller = FALSE order By (xName+'+QuotedStr(' ')+'+xFamily)';
          SelectedField.DisplayFieldName := 'NameFamily';
          lbledSelPerson.Text := SelectedField.Execute;
     end;
     fmDefCustomer.OnKeyUp(Self,Key,Shift)
end;

procedure TfmDefCustomer.btnSelPersonClick(Sender: TObject);
begin
     SelectedField.Query :='Select xPersonCode_Pk_AN,(xName+'+QuotedStr(' ')+'+xFamily) As NameFamily,xIsSeller From tblPerson Where xIsSeller = FALSE order By (xName+'+QuotedStr(' ')+'+xFamily)';
     SelectedField.DisplayFieldName := 'NameFamily';
     lbledSelPerson.Text := SelectedField.Execute;
...
procedure TfmDefCustomer.lbledSelPersonChange(Sender: TObject);
begin
     if Trim(lbledSelPerson.Text) <>'' then
        LoadData(StrToInt(lbledSelPerson.Text));
end;

procedure TfmDefCustomer.btnSaveEditClick(Sender: TObject);
begin
     with TDefPersonClass do
     begin
          ...
          if CheckData(False)Then
              Update(lbledSelPerson.Text,Trim(lbledNameEdit.Text),Trim(lbledFamilyEdit.Text),Trim(lbledJobEdit.Text),cmbTahsilatEdit.Text,Trim(lbledHomeTelEdit.Text),
                     Trim(lbledWorkTelEdit.Text),Trim(lbledMobileEdit.Text),False,chbIsMarridEdit.Checked,chbGendorEdit.Checked,
...
procedure TfmDefCustomer.dbgPersonKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
Var
   thisKey : Word;
...
begin
     ...
     fmDefCustomer.OnKeyUp(Self,thisKey,thisShift);
end;

function TfmDefCustomer.MyShowModal: String;
begin
     ShowModal;
     with fmDMmain.tblPerson do
     begin
